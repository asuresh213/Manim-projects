...
class ArrowHead(Scene):
    def construct(self):
        ...
        for i in range(len(path)):
            ...
            curve = VMobject()
            if(i%2==0):
                curve.set_points_as_corners([*path[i]])
                curve.rotate(angle)
                curve.move_to([0,-2,0], aligned_edge=BOTTOM)
            else:
                curve.set_points_as_corners([*path[i]]).move_to([0,-2,0], aligned_edge=BOTTOM)
            colors = color_gradient(colors, len(curve.get_points()))
            curve.set_color_by_gradient(colors)
            self.play(ReplacementTransform(prev_curve, curve), run_time=4)
            self.wait(0.5)
